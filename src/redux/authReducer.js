   ...
   return async (dispatch,getState)=>{
       ...
       if(!token){
           ...
           if(getState().auth.login!=="admin@gmail.com" && (!getState().auth.group || !getState().auth.fio)){         
                const response = await Axios.get(`https://khai-90ff9.firebaseio.com/Users/.json`)
...
export const auth=(email,password,group,fio,isLogin)=>{
    return async (dispatch,getState)=>{
        ...
        let url=`https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyAfWcZkhpyw1mMSkWSB6MXGLITABWQrGFk`

        if(!isLogin){
            url=`https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyAfWcZkhpyw1mMSkWSB6MXGLITABWQrGFk`;
            const response = await Axios.get(`https://khai-90ff9.firebaseio.com/Users/.json`)
            let lastIdx=0;if(response.data){lastIdx=response.data.length}
            await Axios.put(`https://khai-90ff9.firebaseio.com/Users/${lastIdx}/.json`,{email,group,fio})
        }
        try{
