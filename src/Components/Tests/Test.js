    ...
    onSubmit(e){
        e.preventDefault()
          ...
          this.props.completedTest(state);
              ...
              window.M.toast({html:"Укажите ответы на все вопросы!"})
          }
    }
    ...
    render(){
        ...
        return(
            <div className="container" style={{marginBottom:'3rem'}}>
                <h4 style={{color:'#fff'}}>Тестирование: {this.props.currentTest.titleTest}</h4>
                <form onSubmit={(e)=>this.onSubmit(e)}>
                {this.props.currentTest.test && this.props.currentTest.test.map((q,i)=>{
                return(
                <div key={i} className="row" style={{cursor:'pointer', backgroundColor:'#fff',padding:'10px'}}>
